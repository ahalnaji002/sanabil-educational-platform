...
export function AnnouncementDialog({ announcement, onClose }: { announcement: Announcement | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    dialog.showModal();
    return () => { document.body.style.overflow = ""; if (dialog.open) dialog.close(); previousFocus.current?.focus(); };
  }, [announcement]);
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby="announcement-title" onCancel={(event) => { event.preventDefault(); onClose(); }} onClick={(event) => { if (event.target === dialogRef.current) onClose(); }} className={`m-auto max-h-[85dvh] overflow-y-auto overscroll-contain rounded-[2rem] border-0 bg-white p-0 text-right text-[var(--sanabil-navy)] shadow-2xl backdrop:bg-[#031024]/75 backdrop:backdrop-blur-sm sm:max-h-[92dvh] ${imageUrl ? "w-[calc(100%_-_1rem)] sm:w-[min(calc(100vw-1.5rem),70.4dvh,42rem)]" : "w-[calc(100%_-_1.5rem)] max-w-2xl"}`}>
      {imageUrl ? <div className="relative w-full overflow-hidden bg-white sm:flex sm:aspect-[1117/1170] sm:items-end sm:bg-[var(--sanabil-navy)] sm:text-white">
        ...
        <div className="flex items-start justify-between gap-4"><div>{announcement.badge ? <span className="rounded-full bg-[var(--sanabil-gold-soft)] px-3 py-1 text-xs font-bold">{announcement.badge}</span> : null}<h2 id="announcement-title" className="mt-4 text-2xl font-extrabold sm:text-3xl">{announcement.title}</h2></div><button type="button" onClick={onClose} aria-label="إغلاق الإعلان" className="sticky top-0 flex size-11 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xl focus-visible:outline-2 focus-visible:outline-[var(--sanabil-navy)] sm:static">×</button></div>
        <p className="mt-5 whitespace-pre-line leading-8 text-slate-600">{announcement.content}</p>{cta}
      </div>}
    </dialog>
  );
}
