...
describe("AdminSubjectsPage", () => {
  it("renders subjects and requests server filters", async () => {
    ...
    await waitFor(() => expect(mocks.getSubjects).toHaveBeenLastCalledWith({ status: "inactive", grade: "ELEVENTH" }));
  });
  ...
  it("validates and completes the create flow", async () => {
    ...
    await user.click(within(dialog).getByRole("button", { name: "إضافة المادة" }));
    await waitFor(() => expect(mocks.createSubject).toHaveBeenCalledWith({ name: "الأحياء", slug: "biology", grade: "TAWJIHI", isActive: true }));
  ...
  it("supports edit and confirmed soft-deactivation", async () => {
    ...
    await waitFor(() => expect(mocks.updateSubject).toHaveBeenCalledWith(1, { name: "رياضيات متقدمة", slug: "mathematics", grade: "TAWJIHI", isActive: true }));
