...
import type { AdminSubject, Grade, SubjectStatus } from "../../types/admin-subject";
...
export function AdminDriveLinksPage() {
  ...
  const filters = useMemo(() => ({ status, ...(subjectId ? { subjectId } : {}), ...(grade ? { grade } : {}) }), [grade, status, subjectId]);
  ...
  function changeGrade(nextGrade: Grade | "") {
    setLoading(true);
    setGrade(nextGrade);
    if (nextGrade && selectedSubject?.grade !== nextGrade) setSubjectId(undefined);
  }
  ...
  return <div className="mx-auto max-w-7xl"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-bold text-[var(--sanabil-gold-dark)]">إدارة المحتوى</p><h1 className="mt-1 text-3xl font-black text-[var(--sanabil-navy)]">الروابط التعليمية</h1><p className="mt-2 text-sm leading-7 text-slate-600">أدر روابط Google Drive الخاصة بكل مادة ورتّب ظهورها للطلاب.</p></div><button type="button" onClick={() => { setEditing(null); setFormOpen(true); }} disabled={!subjects.length} className="min-h-11 rounded-xl bg-[var(--sanabil-navy)] px-5 font-extrabold text-white disabled:opacity-50">إضافة رابط</button></div>
    <section aria-label="مرشحات الروابط" className="mt-7 grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 md:grid-cols-3"><div><label htmlFor="links-grade" className="mb-2 block text-sm font-bold">الصف</label><select id="links-grade" value={grade} onChange={(event) => changeGrade(event.target.value as Grade | "")} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-4"><option value="">الكل</option>{gradeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div><div><label htmlFor="links-subject" className="mb-2 block text-sm font-bold">المادة</label><select id="links-subject" value={subjectId ?? ""} onChange={(event) => { setLoading(true); setSubjectId(event.target.value ? Number(event.target.value) : undefined); }} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-4"><option value="">الكل</option>{filteredSubjects.map((subject) => <option key={subject.id} value={subject.id}>{grade ? subject.name : `${subject.name} — ${getGradeLabel(subject.grade)}`}</option>)}</select></div><div><label htmlFor="links-status" className="mb-2 block text-sm font-bold">الحالة</label><select id="links-status" value={status} onChange={(event) => { setLoading(true); setStatus(event.target.value as SubjectStatus); }} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-4">{statuses.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div></section>
    {selectedSubject ? <section className="mt-5 grid gap-3 rounded-2xl bg-[var(--sanabil-navy)] p-5 text-white sm:grid-cols-3"><p><span className="block text-xs text-slate-300">الصف</span><strong>{getGradeLabel(selectedSubject.grade)}</strong></p><p><span className="block text-xs text-slate-300">حالة المادة</span><strong>{selectedSubject.isActive ? "نشطة" : "غير نشطة"}</strong></p><p><span className="block text-xs text-slate-300">عدد الروابط المطابقة</span><strong>{links.length}</strong></p></section> : null}
    ...
    <section aria-busy={loading} aria-live="polite" className="mt-6">{loading ? <p className="rounded-2xl bg-white p-8 text-center font-bold">جارٍ تحميل الروابط...</p> : links.length === 0 ? <p className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center font-bold text-slate-600">لا توجد روابط مطابقة لهذه المرشحات.</p> : <><div className="hidden overflow-hidden rounded-2xl border border-slate-200 bg-white lg:block"><table className="w-full text-right text-sm"><thead className="bg-slate-100"><tr><th className="p-4">الترتيب</th><th className="p-4">العنوان</th><th className="p-4">الوصف</th><th className="p-4">المادة</th><th className="p-4">الحالة</th><th className="p-4">الرابط</th><th className="p-4">الإجراءات</th></tr></thead><tbody className="divide-y divide-slate-200">{links.map((item, index) => <tr key={item.id}><td className="p-4">{item.sortOrder}</td><td className="p-4 font-bold">{item.title}</td><td className="max-w-xs p-4 text-slate-600">{item.description || "—"}</td><td className="p-4">{item.subject.name}<span className="block text-xs text-slate-500">{getGradeLabel(item.subject.grade)}</span></td><td className="p-4"><Badge active={item.isActive} /></td><td className="p-4"><a href={item.driveUrl} target="_blank" rel="noopener noreferrer" className="font-bold text-blue-700">فتح الرابط</a></td><td className="p-4">{actions(item, index)}</td></tr>)}</tbody></table></div><div className="grid gap-4 lg:hidden">{links.map((item, index) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-extrabold">{item.title}</h2><p className="mt-1 text-sm text-slate-600">{item.subject.name} — {getGradeLabel(item.subject.grade)}</p></div><Badge active={item.isActive} /></div><p className="mt-3 text-sm text-slate-600">الترتيب: {item.sortOrder}</p>{item.description ? <p className="mt-3 break-words text-sm">{item.description}</p> : null}<a href={item.driveUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex font-bold text-blue-700">فتح الرابط</a><div className="mt-4">{actions(item, index)}</div></article>)}</div></>}</section>
