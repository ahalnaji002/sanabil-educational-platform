...
export function AdminDriveLinksPage() {
  ...
  return <div className="mx-auto max-w-7xl"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-bold text-[var(--sanabil-gold-dark)]">إدارة المحتوى</p><h1 className="mt-1 text-3xl font-black text-[var(--sanabil-navy)]">الروابط التعليمية</h1><p className="mt-2 text-sm leading-7 text-slate-600">أدر روابط Google Drive الخاصة بكل مادة ورتّب ظهورها للطلاب.</p></div><button type="button" onClick={() => { setEditing(null); setFormOpen(true); }} disabled={!subjects.length} className="min-h-11 rounded-xl bg-[var(--sanabil-navy)] px-5 font-extrabold text-white disabled:opacity-50">إضافة رابط</button></div>
    <section aria-label="مرشحات الروابط" className="mt-7 grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 md:grid-cols-3"><div><label htmlFor="links-grade" className="mb-2 block text-sm font-bold">الصف</label><select id="links-grade" value={grade} onChange={(event) => { setLoading(true); setGrade(event.target.value as Grade | ""); }} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-4"><option value="">الكل</option>{gradeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div><div><label htmlFor="links-subject" className="mb-2 block text-sm font-bold">المادة</label><select id="links-subject" value={subjectId ?? ""} onChange={(event) => { setLoading(true); setSubjectId(event.target.value ? Number(event.target.value) : undefined); }} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-4"><option value="">الكل</option>{subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name} — {getGradeLabel(subject.grade)}</option>)}</select></div><div><label htmlFor="links-status" className="mb-2 block text-sm font-bold">الحالة</label><select id="links-status" value={status} onChange={(event) => { setLoading(true); setStatus(event.target.value as SubjectStatus); }} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-4">{statuses.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div></section>
